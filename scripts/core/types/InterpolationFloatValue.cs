...
namespace Core.Types
{
	...
	public class InterpolationFloatValue : IInterpolationDelta
	{
		...
		public InterpolationFloatValue()
		{
			...
			PositiveDelta = new DefaultValueClass<float>(0f);
			NegativeDelta = new DefaultValueClass<float>(0f);
			TargetValue = 0f;
			CanUpdate = true;
		}

		/// <summary>Конструктор для создания экземпляра InterpolationFloatValue.</summary>
		public InterpolationFloatValue(in ValueRange<float> rangeValues, in IDefaultValue<float> positiveDelta, in IDefaultValue<float> negativeDelta, in float targetValue, in bool canUpdate = true)
		{
